              ...
              <div class="row">
                <div class="col-md-12">
                  ...
                  <div class="text-center">
                    <p class="badge badge-primary p-2" style="font-size: 13px;">LAPORAN PERIODIK MAHASISWA</p>
                  </div>
                  <table id="datatable1" class="table table-striped table-bordered nowrap dataTable no-footer dtr-inline collapsed" style="font-size: 12px; font-weight: 400; color: black; width: 100%;" width="100%" role="grid" aria-describedby="data-table_info">
                    ...
                    <tbody>
                      ...
                      @for ($i=0; $i < count($getmhs); $i++) <tr>
                        ...
                        <td style="text-align: center;">
                          @if (File::exists(public_path('pdf/'.$getmhs[$i][3].'/pdf_laporan_periodik.pdf')))
                          <a href="{{ asset('pdf/'.$getmhs[$i][3].'/pdf_laporan_periodik.pdf') }}" class="btn btn-danger btn-sm" download="Laporan_Periodik_{{$getmhs[$i][2]}}_{{$getmhs[$i][3]}}.pdf" style="text-decoration: none; text-align:center;"><i class="fas fa-file-pdf fa-sm"></i> PDF</a>
                          @else
                          (empty)
                          @endif
                        </td>
...
<!-- /.content -->
@if(is_null($getmhs) == 0)
@for($h=0; $h < count($getmhs); $h++) 
@include('modal.laporan-periodik-d') 
@endfor 
@endif 
@endsection 
